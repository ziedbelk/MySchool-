...
<c:url var="addAction" value="/seance/add" ></c:url>

<form:form action="${addAction}" commandName="seance">
<table>
	<c:if test="${!empty seance.date}">
	<tr>
		<td>
			<form:label path="id">
	...
	<tr>
		<td>
			<form:label path="elementEnseignement">
				matiere: 
			</form:label>
		</td>
		<td>
		    <form:select cssStyle="width:220px"    id="SelectList1" items="${listeElements}" itemLabel="code" itemValue="code"  path="elementEnseignement.code" value="">
...
<%-- 	<tr>
		...
		<td>
		    <form:select  id="SelectList5" items="${listePlans}" itemLabel="code" itemValue="code"  path="planEtude.code" value="">
		    </form:select>
		</td> 
	</tr> --%>
	<tr>
		...
		<td>
		    <form:select cssStyle="width:220px"   id="SelectList2" items="${listeEnseignants}" itemLabel="code" itemValue="code"  path="enseignant.code" value="">
	...
	<tr>
		<td>
			<form:label path="remarque">
				<spring:message text="remarque:"/>
			</form:label>
		</td>
		<td>
	...
	<tr>
		<td colspan="2">
			<c:if test="${!empty seance.date}">
				<input type="submit"
					value="<spring:message text="Edit Seance"/>" />
			</c:if>
			<c:if test="${empty seance.date}">
				<input type="submit"
					value="<spring:message text="Add Seance"/>" />
			</c:if>
		</td>
	</tr>
...
<br>
<c:if test="${!empty listSeances}"><div class="col-sm-12"> 
        	<div class="panel panel-default">
            <div class="panel-heading"><h3>Séances:</h3><input type="text" id="myInput" onkeyup="myFunction()" placeholder="date..">
                                                        <input type="text" id="myInput1" onkeyup="myFunction1()" placeholder="classe.."></div>
            <div class="panel-body">
	<table  id="myTable" class="table" style="border-collapse:collapse;width: auto;">
	<thead>
	<tr>
		<th width="80">ID</th>
		<th width="120">date</th>
		<th width="120">remarque</th>
		<th width="120">enseignant</th>
	...
	<c:forEach items="${listSeances}" var="seance">
		<tr>
			...
			<td>${seance.enseignant.code}</td>
			<td>${seance.elementEnseignement.code}</td>
			<td>${seance.classe.nomClasse}</td>
			<td>${seance.confirmation}</td>
			<td>${seance.dateConfirmation}</td>
			<td><a href="<c:url value='/confirmerSeance/${seance.id}' />" >Confirmer</a></td>
			<td><a href="<c:url value='/editSeance/${seance.id}' />" >Editer</a></td>
			<td><a href="<c:url value='/removeSeance/${seance.id}' />" >Supprimer</a></td>
			<td><a href="<c:url value='/viewAbsences/${seance.id}' />" >Absences</a></td>
		</tr>
	</c:forEach>
	</table>
