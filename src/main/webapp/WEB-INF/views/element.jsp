                ...
                <div class="row">
                    <div class="col-lg-12">
                        ...
                        <ol class="breadcrumb">
                            ...
                            <li class="active">
                                <i class="fa fa-edit"></i> Gestion des unités d'enseignement
                            </li>
                        </ol>
                    </div>
                </div>
...
<c:url var="addAction" value="/element/add" ></c:url>

<form:form action="${addAction}" commandName="element">
<table>
	<c:if test="${!empty element.code}">
	<tr>
		<td>
			<form:label path="id">
	...
	<tr>
		<td>
			<form:label path="nbrHcours">
				<spring:message text="nbrHcours"/>
			</form:label>
		</td>
		<td>
			<form:input cssStyle="width:220px"  path="nbrHcours" class="form-control"/>
		</td>
	</tr>
		<tr>
		<td>
			<form:label path="nbrHtp">
				<spring:message text="nbrHtp"/>
			</form:label>
		</td>
		<td>
			<form:input cssStyle="width:220px"  path="nbrHtp" class="form-control"/>
		</td>
	</tr>
	<%-- <tr>
		<td>
	...
	<tr>
		<td colspan="2">
			<c:if test="${!empty element.code}">
				<input type="submit"
					value="<spring:message text="Edit element enseignement"/>" />
			</c:if>
			<c:if test="${empty element.code}">
				<input type="submit"
					value="<spring:message text="Add element enseignement"/>" />
			</c:if>
		</td>
	</tr>
...
<br>
<c:if test="${!empty elements}"><div class="col-lg-30">
        	<div class="panel panel-default">
            <div class="panel-heading"><h3>Elements enseignement</h3><input type="text" id="myInput" onkeyup="myFunction()" placeholder="Recherche par code.."></div>
            <div class="panel-body">
	<table id="myTable" class="table" style="border-collapse:collapse;">
	<tr>
		<th width="80"> ID</th>
		<th width="120"> Code</th>
		<th width="120">Nombre heures Cours</th>
		<th width="120">Nombre heures Tp</th>
		<th width="60">Editer</th>
		<th width="60">Supprimer</th>
		<th width="60">Chapitres</th>
		<th width="60">Objectifs</th>
	</tr>
	<c:forEach items="${elements}" var="element">
		<tr>
			<td>${element.id}</td>
			<td>${element.code}</td>
			<td>${element.nbrHcours}</td>
			<td>${element.nbrHtp}</td>
		<%-- 	<td>${unite.classe.nomClasse}</td> --%>
			<td><a href="<c:url value='/editElement/${element.id}' />" >Edit</a></td>
			<td><a href="<c:url value='/removeElement/${element.id}' />" >Delete</a></td>
			<td><a href="<c:url value='/viewChapitre/${element.id}' />" >Chapitres</a></td>
			<td><a href="<c:url value='/viewObjectif/${element.id}' />" >Objectifs</a></td>
		</tr>
	</c:forEach>
	</table>
	</div>
