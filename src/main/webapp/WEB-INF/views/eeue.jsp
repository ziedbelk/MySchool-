                ...
                <div class="row">
                    <div class="col-lg-12">
                        ...
                        <ol class="breadcrumb">
                            ...
                            <li class="active">
                                <i class="fa fa-edit"></i> Elements d'enseignement
                            </li>
                        </ol>
                    </div>
                </div>
...
<c:url var="addAction" value="/eeue/add" ></c:url>

<form:form action="${addAction}" commandName="eeue">
<table>

	<tr>
		<td>
			<form:label path="coef">
				<spring:message text="coef"/>
			</form:label>
		</td>
		<td>
			<form:input cssStyle="width:220px"  path="coef" />
		</td> 
	</tr>
	...
	<tr>
		...
		<td>
		    <form:select  id="SelectList" items="${listElements}" itemLabel="code" itemValue="code"  path="elementEnseignement.code" value="">
...
<%-- 	<tr>
		...
		<td>
		    <form:select  id="SelectList" items="${listEnseignants}" itemLabel="code" itemValue="code"  path="enseignant.code" value="">
	...
	<tr>
		...
		<td>
		    <form:select  id="SelectList" items="${listPlans}" itemLabel="code" itemValue="code"  path="planEtude.code" value="">
	...
	<tr>
		<td colspan="2">
			<c:if test="${eeue.id==0}">
				<input type="submit"
					value="<spring:message text="Ajouter Element"/>" />
			</c:if>
			<c:if test="${eeue.id!=0}">
				<input type="submit"
					value="<spring:message text="Mettre à jours Element"/>" />
			</c:if>
		</td>
	</tr>
...
<br>
<h3>Liste Elements</h3>
<c:if test="${!empty eeues}"><div class="col-lg-10">
        	<div class="panel panel-default">
            <div class="panel-heading"><h3>Elements</h3></div>
            <div class="panel-body">
	<table  id="myTable" class="table table-condensed" style="border-collapse:collapse;">
	<tr>
		<th width="80"> ID</th>
		<th width="120"> Matiere</th>
		<th width="120">Coef</th>
	    <th width="120">Plan</th> 
		<th width="120">Enseignants</th>
		<th width="60">Editer</th>
		<th width="60">Supprimer</th>
	</tr>
	<c:forEach items="${eeues}" var="eeue">
		<tr>
			<td>${eeue.id}</td>
			<td>${eeue.elementEnseignement.code}</td>
			<td>${eeue.coef}</td>
			<td>${eeue.planEtude.code}</td>
			<td><a href="<c:url value='/viewEnseignantEeUe/${eeue.id}' />" >Enseignants</a></td>
			<td><a href="<c:url value='/editEeue/${eeue.id}' />" >Edit</a></td>
			<td><a href="<c:url value='/removeEeue/${eeue.id}' />" >Delete</a></td>
		</tr>
	</c:forEach>
	</table>
	</div>
